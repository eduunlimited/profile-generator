...
export function MoveCardsModal({
  ...
  const handleMove = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Could not move cards.");
    } finally {
      setBusy(false);
    }
  };
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-dialog modal-dialog-sm" onClick={(event) => event.stopPropagation()}>
        <div className="modal-header">
          <strong>Move {selectedCount} card{selectedCount === 1 ? "" : "s"}</strong>
          <button type="button" className="tool-btn tool-btn-cyan" onClick={onClose}>
            Close
          </button>
        </div>
        <div className="modal-body">
          <p className="muted move-accounts-hint">Choose a category to move the selected cards into.</p>
