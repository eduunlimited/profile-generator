...
export function cardCategoryIdsInUse(cards: CreditCard[]): Set<string> {
  return new Set(cards.map((card) => card.categoryId || CARD_UNCATEGORIZED_CATEGORY_ID));
}
...
export function categoriesWithCards(categories: CardCategory[], cards: CreditCard[]): CardCategory[] {
  ...
  return sortCardCategories(categories.filter((category) => (counts.get(category.id) ?? 0) > 0));
}
